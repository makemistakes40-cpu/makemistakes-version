    ...
    <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 border-t border-brand-border/60">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {FEATURE_LIST.map((feat, idx) => (
          <Card key={idx} className="flex flex-col justify-between h-full bg-brand-card/40 border border-brand-border/80 hover:border-brand-violet/40 transition-all duration-300">
            <div className="space-y-4">
              <div className="w-12 h-12 rounded-2xl bg-brand-card-light/65 border border-brand-border/80 flex items-center justify-center text-2xl shadow-sm">
                {feat.icon}
              </div>
